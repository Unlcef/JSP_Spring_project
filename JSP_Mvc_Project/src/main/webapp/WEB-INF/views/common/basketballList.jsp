...
<table align="center" style="width:60%">
  <caption style="background-color: rgba(0,0,0,0.2)"><h1>BasketBall</h1></caption>
			...
			<th align="center" colspan="4">
					<c:if test="${listcnt > 0}">
						...
						<c:if test="${startPage > pageBlock}">
							<a href="basketballList.ad">[처음으로]</a>
							<a href="basketballList.ad?pageNum=${startPage - pageBlock}">[이전]</a>
						</c:if>
						<c:forEach var="i" begin="${startPage}" end="${endPage}">
							...
							<c:if test="${i != currentPage}">
								<a href="basketballList.ad?pageNum=${i}">[${i}]</a>
							</c:if>
						</c:forEach>
						
						<!-- 다음블록[▶] / 끝[▶▶] -->
						<c:if test="${pageCount > endPage }">
							<a href="basketballList.ad?pageNum=${startPage + pageBlock}">[다음]</a>
							<a href="basketballList.ad?pageNum=${pageCount}">[끝]</a>
						</c:if>
					</c:if>
				</th>	
			</tr>
